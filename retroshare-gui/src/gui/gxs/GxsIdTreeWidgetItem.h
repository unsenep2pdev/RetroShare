...
class UnseenMemberTreeWidgetItem : public QObject, public RSTreeWidgetItem
{
    Q_OBJECT

public:
    UnseenMemberTreeWidgetItem(const RSTreeWidgetItemCompareRole *compareRole, uint32_t icon_mask,QTreeWidget *parent = NULL);

    void setId(const RsGxsId &id, int column, bool retryWhenFailed);
    bool getId(RsGxsId &id);

    //unseenp2p - add more functions for Unseen member list
    void setMember(const GxsChatMember member, int column, bool retryWhenFailed);
    bool getMember(GxsChatMember &member);
